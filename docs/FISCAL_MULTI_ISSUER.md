...
> ⚠️ **Nota (00023):** `memberships.actor_id` es el **miembro/cliente**, no el
> issuer/tenant. El backfill a continuación usa ese campo solo para preservar
> visibilidad de datos legados. Para nuevos asientos, se requiere modelar
> `issuer_actor_id` o `tenant_actor_id` explícito en membresías.
